...
class Bill extends Controller {
    ...
    async workflowEnter() {
        ...
        let record;
        if (error.code === '0') {
            await ctx.service.bill.workflow.enterWorkflow({idWorkflow, workId}).then(({error: err, record: rec}) => {
                error = err;
                record = rec;
                return rec
            })
    ...
    async workflowAgain() {
        ...
        if (error.code === '0') {
            ...
            await ctx.service.bill.workflow.createWorkflowLog({workId, memo: '重新发起审批流', description: `${user.userName}重新发起审批流`}, 'again',);
            await ctx.service.bill.workflow.enterWorkflow({idWorkflow, workId}).then(({error: err, record: rec}) => {
                error = err;
                record = rec;
                return rec
    ...
    async getWorkflowDesign() {
        ...
        let record;
        if (error.code === '0') {
            await ctx.service.bill.workflow.getWorkflowDesignByWorkId(workId).then(({error: err, record: rec}) => {
                error = err;
                record = rec;
                return rec
            })
    ...
    async getWorkflowLog() {
        ...
        let records;
        if (error.code === '0') {
            const {nodes, edges} = await ctx.service.bill.workflow.getWorkflowDesignByWorkId(workId).then(r => r.record);
            ...
            const activeEdges = await ctx.model.WfWorkflowActiveLog.find({workId,__s:1}).populate(['idUser', 'idWorkflowMember']).lean().then(records => {
                return records.map(item => {
                    return {
                        ...
                        esKey: ctx.service.bill.workflow.countSignKey({...item, id: item.idWorkflowMember.id})
                    }
                })
            });
    ...
    async getWorkflowPointUsers() {
        ...
        if (error.code === '0') {
            ...
            const {nodes} = await ctx.service.bill.workflow.getWorkflowDesignByWorkId(workId).then(r => r.record);
    ...
    async workflowVerify() {
        ...
        if (error.code === '0') {
            /* 判断当前节点是否为 激活节点*/
            const {activeNodes, activeEdges} = await ctx.service.bill.workflow.getActiveWorkflowByWorkId(workId).then(res => {
                return res.error.code === '0' ? res.record : {activeEdges: [], activeNodes: []};
            });
            if (activeNodes.includes(source)) {
                /* 普通模式 */
                const result = await ctx.service.bill.workflow.mainWorkflow({workId,operationNodeId:source, idUser, pointSigUser, memo, operationType})
            } else {
                error.code = '10104';
                error.message = '当前节点未激活,该节点已被审核或已关闭,请刷新后重试！'
            }
    ...
    async workflowReject() {
        ...
        if (error.code === '0') {
            /* 判断当前节点是否为 激活节点*/
            const {activeNodes} = await ctx.service.bill.workflow.getActiveWorkflowByWorkId(workId).then(res => {
                return res.error.code === '0' ? res.record : {activeEdges: [], activeNodes: []};
            });
            if (activeNodes.includes(source)) {
                await ctx.service.bill.workflow.mainWorkflow({workId,operationNodeId: source, idUser, memo, operationType})
            } else {
                error.code = '10104';
                error.message = '当前节点未激活,该节点已被审核或已关闭,请刷新后重试！'
            }
    ...
    async workflowCounterSig() {
        ...
        if (error.code === '0') {
            ...
            if (activeNodes.includes(source)) {
                ...
                if (edges[0]) {
                    await ctx.service.bill.workflow.createWorkflowLog({
                ...
                for (let edge of edges) {
                    for (let user of users) {
                        if (!activeEdges.map(ae => ae.esKey).includes(ctx.service.bill.workflow.countSignKey({
                            idUser: user,
                            workId,
                            id: edge.id
                        }))) {
    ...
    async workflowChangeSig() {
        ...
        if (error.code === '0') {
            ...
            if (activeNodes.includes(source)) {
                ...
                /* 改派数据*/
                for (let edge of edges) {
                    await ctx.service.bill.workflow.closureActiveLog({
                        workId,
                        id:edge.id,
                        idUser,
