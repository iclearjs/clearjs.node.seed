...
class CorePageCtrl extends Controller {
  constructor(ctx) {
    super(ctx);
  ...
  async export() {
    ...
    if (error.code === '0') {
      ...
      filter = ctx.service.coreHelper.toAggregateFilters(await this.ctx.service.coreHelper._setUserAuthFilter(filter, pageConfig.idEntityList.dsCollection));
      ...
      records = await ctx.model[this.ctx.helper.humps.pascalize(pageConfig.idEntityList.dsCollection)].aggregate([ ...ctx.helper.toObjectIDs(prePipeline), ...ctx.service.coreHelper.getAggregatePaging({
        order,
        skip: page ? skip : 0,
        limit: page ? limit : 0,
