...
class CoreWorkflow extends Controller {
  constructor(ctx) {
    super(ctx);
  }
  ...
  async workflowEnter() {
    const { ctx } = this;
    let error = { code: '0' };
    const { idWorkflow, workId } = ctx.request.body;
    if (!idWorkflow) {
      error.code = '10104';
  ...
  async workflowAgain() {
    const { ctx } = this;
    let error = { code: '0' };
    const { workId, idWorkflow, idUser } = ctx.request.body;
    if (!workId) {
      error.code = '10104';
      error.message = 'params workId missing';
    }
  ...
  async getWorkflowActiveNodes() {
    ...
    let records = [];
    if (error.code === '0') {
      records = await this.ctx.model.WfWorkflowActiveNodes.find({ workId }).populate([ 'idWorkflowMember' ]).lean()
        .then(records => {
  ...
  async getWorkflowLog() {
    ...
    if (error.code === '0') {
      ...
      records = await ctx.model.WfWorkflowActiveLog.find({ workId, __s: { $ne: 1 }, operationType: { $ne: 'close' } }).sort('-operationAt').populate([ 'idUser' ])
        .lean();
  ...
  async getWorkflowAuthButtons() {
    const { ctx } = this;
    const error = { code: '0' };
    const { workId, source, idUser } = ctx.request.body;
    // pointSigUser
    if (!workId) {
      error.code = '10104';
      error.message = 'params workId missing';
    ...
    if (error.code === '0') {
      /* 判断当前节点是否为 激活节点*/
      const activeEdges = await ctx.model.WfWorkflowActiveLog.find({ workId, __s: 1 }).populate([ 'idUser', 'idWorkflowMember' ]).lean()
        .then(records => {
          return records.map(item => {
            return {
      ...
      const activeNodes = await ctx.model.WfWorkflowActiveNodes.find({ workId }).populate([ 'idWorkflowMember' ]).lean()
        .then(records => {
          return records.map(item => {
            return item.idWorkflowMember;
          });
      ...
      if (activeNodes.map(el => el.id).includes(source)) {
        /* 普通模式 */
        const edges = activeEdges.filter(item => item.source === source && idUser === item.idUser._id.toString());
        const node = activeNodes.filter(el => el.id === source)[0];
        if (edges.length > 0) {
  ...
  async getWorkflowPointUsers() {
    ...
    if (error.code === '0') {
      /* 判断当前节点是否为 激活节点*/
      const activeEdges = await ctx.model.WfWorkflowActiveLog.find({ workId }).populate([ 'idWorkflowMember' ]).lean()
        .then(records => {
          return records.map(item => {
            return {
              idWorkflowActiveEdge: item._id,
      ...
      const idActiveEdges = activeEdges.filter(item => item.source === source && idUser === item.idUser.toString()).map(el => el.target);
  ...
  async workflowVerify() {
    ...
    const { workId, source, idUser, memo, pointSigUser } = ctx.request.body;
    // pointSigUser
    if (!workId) {
      error.code = '10104';
      error.message = 'params workId missing';
  ...
  async workflowReject() {
    ...
    const { workId, source, idUser, memo } = ctx.request.body;
    if (!workId) {
      error.code = '10104';
      error.message = 'params workId missing';
    }
  ...
  async workflowCounterSig() {
    const { ctx } = this;
    const error = { code: '0' };
    const { workId, source, idUser, users, memo } = ctx.request.body;
    if (!workId) {
      error.code = '10104';
      error.message = 'params workId missing';
    }
  ...
  async workflowChangeSig() {
    const { ctx } = this;
    const error = { code: '0' };
    const { workId, source, idUser, toUser, memo } = ctx.request.body;
    if (!workId) {
      error.code = '10104';
      error.message = 'params workId missing';
    }
