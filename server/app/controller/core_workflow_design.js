...
class Form extends Controller {
    ...
    async saveAsOtherWorkFlow() {
        const {ctx} = this;
        const error = {code: '0'};
        let {nodes, edges, record} = ctx.request.body;
    ...
    async workFlowHasBillIn() {
        const {ctx} = this;
        const error = {code: '0'};
        let {idWorkflow: _id} = ctx.request.query;
    ...
    async deleteWorkFlowAndMembers() {
        ...
        if (error.code === '0') {
            ...
            record = await ctx.model.WfWorkflow.findOne({_id}).lean();
            const {CtxModel} = await ctx.service.bill.index.GetPageParams({referName: record.idPage});
            const WorkflowBill = CtxModel.findOne({idWorkflow:record._id});
    ...
    async getWorkFlowMembers() {
        const {ctx} = this;
        const error = {code: '0'};
        let {idWorkflow} = ctx.request.query;
        if (!idWorkflow) {
            error.code = '10104';
            error.message = 'params idWorkflow missing'
        }
    ...
    async saveWorkFlowMembers() {
        const {ctx} = this;
        const error = {code: '0'};
        let {nodes, edges, idWorkflow} = ctx.request.body;
        if (!idWorkflow) {
            error.code = '10104';
            error.message = 'params idWorkflow missing'
        }
