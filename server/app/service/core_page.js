...
class CorePage extends Service {
        ...
        for (const j in FileData[i]) {
          ...
          if (nameField && nameField.field) {
            switch (nameField.widget) {
              case 'RadioRefer':
                if (Number(i) === 1) {
                  translateMap[nameField.field] = await translateRefField(nameField.idRefer, {
                    key: nameField.referDisplay, values: FileData.reduce((sum, item, index) => {
                      if (Number(index) === 0) {
                        return [];
                      }
                      return [ ...sum, item[j] ];
  ...
  async doRevoke(_id, operateUser, pageIdOrCode) {
    ...
    if (error.code === '0') {
      ...
      /** @pushMessage REVOKE 取消待审核状态 通知有审批权限的人员 告知该单据撤回修改 */
      const sendTo = await this.service.coreHelper.getPageAuthUsers(PageConfig._id, 'verify', { idOrgan: PageData.idOrgan });
