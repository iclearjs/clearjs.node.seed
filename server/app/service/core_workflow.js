...
class SystemWorkflow extends Service {
    constructor(ctx) {
        super(ctx);
    }
    ...
    * */
    ...
    async enterWorkflow({workId, idWorkflow}, option = {}) {
        ...
        if (error.code === '0') {
            ...
            if (isEnterWorkflow) {
                ...
                let start = nodes.filter(res => res.nodeType === 'start')[0];
                let submit = nodes.filter(res => res.nodeType === 'submit')[0];
                let {_id,...edge} = edges.filter(res => res.source === start.id && res.target === submit.id)[0];
                await this.ctx.model.WfWorkflowActiveNodes.create({idWorkflowMember:start._id,id:start.id, workId, label: start.label});
                await this.ctx.model.WfWorkflowActiveLog.create({idWorkflowMember:_id,...edge, workId});
                const resultError = await this.mainWorkflow({operationNodeId:start.id, memo: '审批开始',workId}, {nodes, edges, workflowParams})
    ...
    async mainWorkflow(body, WorkflowDesign) {
        ...
        * */
        ...
        if(operationEdges.length > 0){
            ...
            if (pointSigUser) {
                const {User, pUser} = await this.ctx.model.SysUser.find({_id: {$in: [pointSigUser, idUser]}}).lean().then(el => {
                    return {
        ...
        if(operationNode.preemptType === 'countersign'){
            let countersignValue = operationNode.countersignValue / 100;
            /* 获取所有会签 数据 */
            const logs = await this.ctx.model.WfWorkflowActiveLog.find({source:operationNode.id,workId,
                __s:{ $in:[1,2]}
            });
        ...
        /* 节点审批结束 */
        /* 清除 当前路径上其他人员的审批 任务 */
        await this.closureActiveLog({source: operationNode.id,__s:1,workId, idUser: {$ne: idUser}}, {operationAt: new Date()});
        ...
        const enterWorkNode = async (workNodeParams, WorkflowDesign) => {
            ...
            let error = {code: '0'};
            let node = WorkflowDesign.nodes.filter(item => item.id === id)[0];
            ...
            if (node.mergeType === 'and') {
                ...
                const getParentNodes = ({nodes, edges}, node) => {
                    ...
                    for (let edge of edges) {
                        if (edge.target === node && edge.resultType !== 'NO') {
                            let source = nodes.filter(item => item.id === edge.source);
                ...
                let sourceEdges = parent.filter(item => activeNodes.includes(item.id));
                ...
                /* 关闭其余父节点*/
                let otherTargetEdges = edges.filter(row => row.source !== sourceNode.id && row.target === node.id).map(item => item.source);
                for (let ed of otherTargetEdges) {
        ...
        for(let edge of operationEdges){
    ...
    async judgeAllowCond({workId, source, operationType = 'accept'}, WorkflowDesign) {
        let error = {code: '0'};
        let records = [];
        let {nodes, edges, workflowParams} = WorkflowDesign ? WorkflowDesign : await this.getWorkflowDesignByWorkId(workId).then(res => {
            if (res.error.code !== '0') {
                error = res.error
            }
            return res.record
        ...
        if (confEdges.length > 0) {
            ...
            for (let edge of confEdges) {
                ...
                //         break;
                //     case 'mongodb':
                //         data = await http.post('/v1/getByAggregate/' + edge.idPage.table, {
                //             filter:edge.filter,
    ...
    async closureActiveLog(filter, updateParams = {}) {
        ...
        if (error.code === '0') {
            ...
            this.removeOverdueDaysList({idWorkflowMember:records.map(el=>el.idWorkflowMember),workId:filter.workId});
    ...
    async generateActivesLogById(row, WorkflowDesign) {
        ...
        const {nodes, edges} = WorkflowDesign ? WorkflowDesign : await this.getWorkflowDesignByWorkId(workId).then(res => {
            if (res.error.code !== '0') {
                error = res.error
            }
            return res.record
        });
        let {_id:idWorkflowMember,...edge} = edges.filter(item => item.id === id)[0];
        if (!edge) {
            return {error}
        }
        let node = nodes.filter(item => item.id === edge.source)[0];
        if (error.code === '0') {
    ...
    async pushOverdueDaysList(edge, users, WorkflowDesign) {
        ...
        let {overdueNoticeDays, overdueDays} = nodes.filter(item => item.id === edge.target)[0];
        if (!workId) {
            return {error}
        }
        if (error.code === '0') {
    ...
    async pushApprovalMessage({workId, userIds=[],event='WORKFLOW_WAIT'},PageParams) {
        let error = {code: '0'};
        if(!PageParams){
            const idPage = await this.ctx.model.WfWorkflowActive.findOne({ workId }).deepPopulate(['idWorkflow']).lean().then(el=>el?el.idWorkflow.idPage:'');
            PageParams = await this.ctx.service.bill.index.GetPageParams({referName:idPage})
        }
        if (error.code === '0' && PageParams && PageParams.CtxModel) {
            const bill = await PageParams.CtxModel.findOne({_id:workId}).lean();
            ...
            await this.ctx.service.msg.push.pushMessage(event,{...bill,users:userIds,workId,},PageParams.Page);
        }else{
            error.code = '504';
    ...
    async getActiveWorkflowByWorkId(workId) {
        ...
        let record = {activeEdges: [], activeNodes: []};
        if (error.code === '0') {

            record.activeEdges = await this.ctx.model.WfWorkflowActiveLog.find({
                workId,
                __s: 1
            }).populate(['idUser', 'idWorkflowMember']).lean().then(records => {
            ...
            record.activeNodes = await this.ctx.model.WfWorkflowActiveNodes.find({workId}).populate(['idWorkflowMember']).lean().then(records => {
                return records.map(item => {
    ...
    async endWorkflow(workId, {node, closeMemo}, operationType = 'end') {
        ...
        if (operationType === 'end') {
            ...
            const operateUser = await this.ctx.model.WfWorkflowActiveLog.findOne({workId,idUser:{$exists:true},$and:[{operationType:{$exists:true}},{operationType:{$nin:['close']}}],__s:2}).sort('-operationAt').then(el=>{
                return el.idUser;
            });
            const {CtxModel,Page} = await this.ctx.service.bill.index.GetPageParams({referName: workflow.idPage});
            let bill = await CtxModel.find({_id: workId});
            if (bill && bill.length === 1) {
                switch (bill[0].__s) {
                    case 2:
                        error = (await this.ctx.service.bill.index.doVerify({ _id: bill[0]._id,operateUser: operateUser}, {referName: workflow.idPage})).error;
                        await this.pushApprovalMessage({workId,userIds: [],event:'WORKFLOW_END'},{CtxModel,Page});
                        break;
                    case 3:
                        error.code = '0';
