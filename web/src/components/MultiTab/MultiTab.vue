<template>
  <a-tabs hideAdd :activeKey="menu.key" type="editable-card" @edit="onTabEdit" @change="onTabChange"
    ...
    <a-row type="flex" justify="center" align="middle" slot="tabBarExtraContent">
      <a-tree-select v-if="menu.controlType==='Organ'" :value="organ" style="width:200px"
                     :dropdown-style="{ maxHeight: '400px', overflow: 'auto' }" :tree-data="menu.organs"
                     placeholder="选择组织" tree-default-expand-all @select="onOrganChange"></a-tree-select>
  ...
  created() {
    if (this.getTabIndex(this.menu.key) < 0) {
      if (!this.menu.idOrgan && this.menu.organs && this.menu.organs.length) {
        this.ToggleMenu({...this.menu, idOrgan: this.menu.organs[0]._id})
        this.SetOrgan(this.menu.organs[0]._id)
